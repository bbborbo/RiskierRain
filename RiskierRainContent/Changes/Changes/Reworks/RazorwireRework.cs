...
namespace RiskierRainContent
{
    public partial class RiskierRainContent
	{
        ...
        private void RazorwireBegin(ILContext il)
		{
			...
			c.EmitDelegate<Func<Int32, HealthComponent, Int32>>((itemCount, hc) =>
			{
				...
				buffCount = 0;
				while (body.HasBuff(CoreModules.Assets.razorChargeBuff))
				{
					body.RemoveBuff(CoreModules.Assets.razorChargeBuff);
					buffCount++;
				}
				if (buffCount <= 0)
				{
	...
	public class RazorwireBehavior : CharacterBody.ItemBehavior
	{
		...
		BuffDef razorBuff => CoreModules.Assets.razorChargeBuff;
