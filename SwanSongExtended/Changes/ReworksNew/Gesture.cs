...
{
    public class Gesture : ReworkBase<Gesture>
    {
        ...
        public override string ItemPath => RoR2BepInExPack.GameAssetPaths.Version_1_39_0.RoR2_Base_AutoCastEquipment.AutoCastEquipment_asset;

        public override string ItemName => "Gesture of the Drowned";
        ...
        private void AddGestureUndercast(On.RoR2.EquipmentSlot.orig_OnEquipmentExecuted orig, EquipmentSlot self)
        {
            ...
            {
                self.characterBody.AddBuff(gestureQueueEquipBreak);
        ...
        public static void TryGestureEquipmentBreak(EquipmentSlot self)
        {
            if (!NetworkServer.active)
                return;
            if (self.characterBody.HasBuff(gestureQueueEquipBreak))
            {
                if (Util.CheckRoll(gestureEquipBreakChance))
                {
                    self.inventory.SetEquipmentIndex(EquipmentIndex.None);
                }
                self.characterBody.RemoveBuff(gestureQueueEquipBreak);
            }
        }
