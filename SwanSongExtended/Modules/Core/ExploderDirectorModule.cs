    ...
    public class ExploderSpawnNode : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            while (spawnTimer > spawnInterval && spawnQueue > 0)
            {
                ...
                spawnQueue -= 1;
                Debug.Log($"Spawn {SwanSongPlugin.exploderCard.name} at position {this.transform.position}!. Remaining queue: {spawnQueue}");

                DirectorSpawnRequest directorSpawnRequest = new DirectorSpawnRequest(SwanSongPlugin.exploderCard, new DirectorPlacementRule
                {
